	...
	public void doTag() throws JspException {
		...
		try {
			...
			for (int itr = 0; itr < count; itr += steps) {
				if (offset == itr)
					out.write(constructLink((itr / 10 + 1) - 1 * steps, 
							String.valueOf(itr / 10 + 1), "active", true));
				else
					out.write(constructLink(itr / 10 * steps, 
							String.valueOf(itr / 10 + 1), null, false));
			}
	...
	private String constructLink(int page, String text, String className, 
		...
		if (disabled)
			...
			if (groupId != null && groupName != null){
				link.append(">").append("<a href=\"" + uri + "?offset=" + page 
